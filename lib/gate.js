...
import {
  DEGRADING_KINDS, classifyFailure, clearDegraded, enterDegraded, isDegraded, probeDue, readDegraded, warningLine,
} from './quota.js'
...
/** The validated noul question. Chinese input was measured to be as good or better than translated English. */
...
async function readSmallText(path, maxBytes) {
  if (SENSITIVE_PATH.test(path)) return { skipped: '疑似敏感文件，未上传内容' }
  try {
    const info = await stat(path)
    if (!info.isFile()) return { skipped: '不是普通文件' }
    if (info.size > maxBytes) return { skipped: `文件过大 (${info.size} bytes)` }
    const text = await readFile(path, 'utf8')
    if (text.includes('\u0000')) return { skipped: '二进制内容' }
    return { text }
  } catch (error) {
    return { skipped: `读取失败: ${error?.code ?? error?.message ?? error}` }
  }
}
...
export async function buildState(command, cfg = {}) {
  ...
  if (pkgName !== undefined) {
    ...
    if (body !== undefined) {
      state.包脚本 = `"${pkgName}": ${body}`
      const inner = scriptTarget(body)
      if (inner !== undefined) {
        const innerPath = isAbsolute(inner) ? inner : resolvePath(cwd, inner)
        const read = await readSmallText(innerPath, maxBytes)
        if (read.text !== undefined) state.包脚本内容 = truncate(read.text, maxBytes)
      }
    }
  }
  ...
  if (target !== undefined) {
    ...
    const read = await readSmallText(path, maxBytes)
    if (read.text !== undefined) state.脚本内容 = truncate(read.text, maxBytes)
    else if (read.skipped !== undefined) state.脚本内容 = `(未读取: ${read.skipped})`
  }
  return state
}
...
function degradedVerdict(state, cfg, now, probe = false) {
  return {
    ...
    reason: `降级放行(${state.kind}):联网语义判定暂停,${cfg.degradePolicy === 'off' ? '整条阀门暂停' : 'L0 免费规则与预筛仍在工作'}`,
    probe,
    ms: 0,
  }
}
...
export async function evaluateCommand(command, options = {}) {
  ...
  const degradedState = cfg.quotaGuard === false ? null : await readDegraded(cfg)
  const degradedNow = isDegraded(degradedState, now)
  ...
  if (degradedNow && !probeDue(degradedState, now)) return degradedVerdict(degradedState, cfg, now)
  ...
  const isProbe = degradedState !== null && probeDue(degradedState, now)

  const started = Date.now()
  let state = { 命令: command }
  try {
  ...
  try {
    ...
    const verdict = {
      ...
      enriched: Object.keys(state).filter(k => k !== '命令'),
      ms: Date.now() - started,
    ...
    if (DEGRADING_KINDS.includes(kind)) {
      const after = await enterDegraded(kind, { cfg, now: Date.now(), detail, status, probe: isProbe })
      return {
        ...base,
