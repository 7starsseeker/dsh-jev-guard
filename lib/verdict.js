...
const TEMPLATE_IDS = Object.freeze(['dry-run', 'narrow', 'backup-first'])

/**
 * The three downgrade templates in the language in force.
 *
 * Text lives in the message catalog (it is read by the model and by a human in an
 * approval dialog), the shape stays here.
 *
 * @param lang - language; defaults to the one currently set.
 * @returns `[{ id, text, examples }]`.
 */
export function templates(lang = getLang()) {
  return TEMPLATE_IDS.map((id) => {
    const examples = []
    for (let i = 0; ; i += 1) {
      const key = `template.${id}.example.${i}`
      const value = t(key)
      if (value === key) break // no such example in any language
      examples.push(value)
    }
    return { id, text: t(`template.${id}.text`), examples }
  })
}
...
export function shellName(platform = process.platform) {
  return platform === 'win32' ? 'PowerShell' : t('verdict.shell.posix')
}

/**
 * @typedef {{ action: 'allow'|'revise'|'block'|'escalate', source: string, p?: number,
 *   model?: string, ms?: number, rule?: {id: string, why: string, kind?: 'deny'|'ask'},
 *   reason?: string, templates?: ReturnType<typeof templates>, enriched?: string[],
 *   error?: string, token?: string, warning?: string }} Verdict
 */
...
export function explain(command, verdict, opts = {}) {
  ...
  const detail = verdict.rule
    ? t('verdict.detail.rule', { id: verdict.rule.id, why: verdict.rule.why })
    // 降级放行**不是**预筛判定出来的,不能复用那句文案 —— 否则读者会以为这条被确认过无害。
    : verdict.source === 'degraded' ? t('verdict.detail.degraded')
      : verdict.p === undefined ? t('verdict.detail.prefilter') : t('verdict.detail.probability', { pct })
  ...
  const askWait = t('verdict.askWait')
  const head = {
    allow: t('verdict.head.allow'),
    revise: prompted ? t('verdict.head.prompted') : t('verdict.head.revise'),
    block: prompted ? t('verdict.head.prompted') : t('verdict.head.block'),
    escalate: t('verdict.head.prompted'),
  }[verdict.action] ?? verdict.action
  const tail = {
    allow: '',
    revise: prompted ? askWait : t('verdict.tail.revise'),
    // 带 L0 `deny` 规则的 block 永远不会转人工,所以这里再挡一层:万一调用方传错,
    // 也不能对着一条不可逆的硬规则说"等用户在弹窗里决定"。
    block: prompted && verdict.rule?.kind !== 'deny' ? askWait : t('verdict.tail.block'),
    escalate: prompted ? askWait : t('verdict.tail.escalate'),
  }[verdict.action] ?? ''
  const auto = opts.policy === 'never' && verdict.action !== 'allow' ? t('verdict.autoNote') : ''
  ...
  const token = opts.token && verdict.action !== 'allow' && verdict.rule?.kind !== 'deny' && !prompted
    ? t('verdict.tokenHint', {
      winNote: platform === 'win32' ? t('verdict.winNote') : '',
      cli: opts.cliPath || DEFAULT_CLI_PATH,
      quoted: shellQuote(command, platform),
      token: opts.token,
    })
    : ''
  return t('verdict.line', {
    head,
    detail,
    tail,
    auto,
...
export function reviseGuidance(command, verdict, opts = {}) {
  const lines = [explain(command, verdict, opts), '', t('verdict.reviseHeading')]
  // 循环变量不要叫 `t` —— 它会遮住 i18n 的查表函数。
  for (const tpl of templates()) {
    lines.push(t('verdict.templateLine', { text: tpl.text, examples: tpl.examples.join(' / ') }))
  }
  return lines.join('\n')
}
