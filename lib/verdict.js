...
export function explain(command, verdict, opts = {}) {
  ...
  // 宿主真的会弹审批框时(action=escalate 且策略为 ask,见 toHostDecision),这句理由的
  // 读者是**弹窗**里的人和后面可能看到拒绝的模型 —— 不能说"本会话没有审批提示"。
  // 实测(2026-09-20,ask 模式)弹窗里原样显示的就是这句旧文案,指着眼前的弹窗说没有弹窗,
  // 还让用户复制一条终端命令 —— 所以这里按策略分叉。
  const prompted = opts.policy === 'ask' && verdict.action === 'escalate'
  const tail = {
    allow: '',
    revise: '请改用下面任一更安全的形式后重试;若都不适用,请向用户说明并请求人工介入。',
    block: '这条命令在本机被禁止自动执行。请改用安全替代方案,或由用户手动执行。',
    escalate: prompted
      ? '这条命令需要人工确认,宿主已向用户发起审批请求 —— 请等用户在弹窗里决定,不要换写法重试。'
 ...
 * @param opts - `{ token, cliPath }`: 一次性放行令牌,会被写进理由里(见 token.js);
 *   `cliPath` 透传给 {@link explain}(见那里的绝对路径说明)。
...
export function toHostDecision(command, verdict, approvalPolicy = 'ask', opts = {}) {
  ...
  if (verdict.action === 'escalate') {
    return approvalPolicy === 'ask' ? { kind: 'ask', reason } : { kind: 'deny', reason }
  }
  // revise and block are both model-facing denials; revise additionally teaches.
