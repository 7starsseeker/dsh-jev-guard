...
|---|---|---|
| `allow` | 放行 | 放行 |
| `revise` | **拒绝** + 三种降级模板(教模型换写法) | 同左,**并附**一次性令牌提示 |
| `block` | **拒绝** | 同左,**并附**一次性令牌提示 |
| `escalate` | **`ask`** —— 宿主弹审批框,由人决定 | **拒绝** + 一次性令牌提示 |

两条容易搞错、但很重要的细节:

1. **只有 `escalate` 会真的走到人面前。** `revise`/`block` 在两种策略下都是拒绝 ——
   它们是"机器已经能下结论"的两态,没有值得打断人的信息。
